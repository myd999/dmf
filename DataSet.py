...
class DataSet(object):
    def __init__(self, fileName, splitSign):
        self.data, self.shape = self.getData(fileName, splitSign)
        self.train, self.test = self.getTrainTest()
        self.trainDict = self.getTrainDict()

    def getData(self, fileName, splitSign):
        print("Loading "+ fileName + " data set...")
        data = []
        if fileName == 'ml-1m':
